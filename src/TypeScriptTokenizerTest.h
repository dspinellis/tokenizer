...
public:
	void testKeyword() {
		...
		CPPUNIT_ASSERT_EQUAL((int)Keyword::K_await, ct.get_token());
		...
		CPPUNIT_ASSERT_EQUAL((int)Keyword::K_Boolean, ct4.get_token());
	...
	void testEQUAL_STRICT() {
		JavaScriptTokenizer ct("===3");
		CPPUNIT_ASSERT_EQUAL((int)Token::EQUAL_STRICT, ct.get_token());
	}

	void testNOT_EQUAL_STRICT() {
		JavaScriptTokenizer ct("!===3");
		CPPUNIT_ASSERT_EQUAL((int)Token::NOT_EQUAL_STRICT, ct.get_token());
	}
	...
	void testBOOLEAN_AND_EQUAL() {
		JavaScriptTokenizer ct("&&=3");
		CPPUNIT_ASSERT_EQUAL((int)Token::BOOLEAN_AND_EQUAL, ct.get_token());
	}

	void testBOOLEAN_OR_EQUAL() {
		JavaScriptTokenizer ct("||=3");
		CPPUNIT_ASSERT_EQUAL((int)Token::BOOLEAN_OR_EQUAL, ct.get_token());
	}

	void testNULL_COALESCING() {
		JavaScriptTokenizer ct("??a");
		CPPUNIT_ASSERT_EQUAL((int)Token::NULL_COALESCING, ct.get_token());
	}

	void testNULL_COALESCING_EQUAL() {
		JavaScriptTokenizer ct("?" "?=a"); // Avoid trigraph
		CPPUNIT_ASSERT_EQUAL((int)Token::NULL_COALESCING_EQUAL, ct.get_token());
	}
	...
	void testSimpleString() {
		...
		CPPUNIT_ASSERT_EQUAL((int)TokenId::IDENTIFIER, ct3.get_token());
		CPPUNIT_ASSERT_EQUAL((int)Token::STRING_LITERAL, ct3.get_token());
		CPPUNIT_ASSERT_EQUAL((int)Keyword::K_in, ct3.get_token());
	}
